...
import { TouristSpot, OpeningHours } from '../src/types';

// Parse operating hours from the tokyo.txt format
function parseOpeningHours(hoursText: string): OpeningHours {
...
function transformCategory(types: string[]): TouristSpot['category'] {
...
async function importTokyoSpots() {
  ...
  try {
    for (const spot of tokyoSpots) {
      ...
      await setDoc(doc(database, 'tourist_spots', spot.placeId), touristSpot);
